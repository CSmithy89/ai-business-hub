...
|----|-------------|----------|-------|
| TD-21 | WebSocket room cleanup comprehensive testing | Medium | room.join/leave handlers added but need E2E verification |
| TD-22 | State sync conflict resolution edge cases | Medium | Current version-based detection may miss rapid concurrent edits |
| TD-23 | Metrics auth key rotation documentation | Low | Need runbook for rotating METRICS_API_KEY |
| TD-24 | Token counter accuracy validation | Low | tiktoken fallback estimation needs validation against real usage |
...
|---|--------|-------|----------|--------|
| 1 | Create security review checklist | Dev Team | Before next epic | Pending |
| 2 | Document async primitive patterns | Dev Team | Before next epic | Pending |
| 3 | Add comment hygiene to PR template | Dev Team | Before next epic | Pending |
| 4 | Update DEPLOYMENT.md with WebSocket security | Dev Team | Before merge | Pending |
| 5 | Validate token counter accuracy in production | Ops | After deploy | Pending |
| 6 | Test room.join/leave handlers E2E | QA | After deploy | Pending |
