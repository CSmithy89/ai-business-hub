...
### Code Review Findings (PR #49 Round 5)

#### Critical Issues

| ID | Description | Priority | File | Status |
|----|-------------|----------|------|--------|
| CR-01 | ~~Timing attack in metrics auth~~ | Critical | `agents/api/middleware/metrics_auth.py:82-85` | ✅ Fixed (0f0b3246) |
| CR-02 | ~~Race condition in dashboard state Lua script~~ | Critical | `apps/api/src/modules/dashboard/dashboard-state.service.ts` | ✅ Fixed (a58e4372) |
| CR-03 | ~~Missing workspace-scoped admin check~~ | Critical | `apps/api/src/approvals/approvals.service.ts` | ✅ Verified correct (a58e4372) |
| CR-04 | ~~Pending changes lost on disconnect~~ | Critical | `apps/web/src/lib/realtime/state-sync-client.ts` | ✅ Intentional design |

#### High Priority Issues

| ID | Description | Priority | File | Status |
|----|-------------|----------|------|--------|
| CR-05 | ~~N+1 lock acquisition in cleanup~~ | High | `agents/hitl/approval_events.py:296-308` | ✅ False positive |
| CR-06 | ~~Missing backpressure in MCP parallel connections~~ | High | `agents/mcp/client.py` | ✅ Fixed (a58e4372) |
| CR-07 | ~~Memory leak - no periodic cleanup task~~ | High | `agents/main.py` | ✅ Fixed (a58e4372) |

#### Medium Priority Issues

| ID | Description | Priority | File | Status |
|----|-------------|----------|------|--------|
| CR-08 | ~~Missing compression metrics to analytics~~ | Medium | `apps/web/src/lib/telemetry/metrics-tracking.ts` | ✅ Fixed (a58e4372) |
| CR-09 | ~~No retry logic for Redis failures~~ | Medium | `apps/api/src/modules/dashboard/dashboard-state.service.ts` | ✅ Fixed (a58e4372) |
| CR-10 | ~~Missing context in cancellation audit logs~~ | Medium | `apps/api/src/approvals/approvals.service.ts` | ✅ Fixed (a58e4372) |

#### Testing Gaps

| ID | Description | Priority | Notes | Status |
|----|-------------|----------|-------|--------|
| CR-11 | ~~Multi-tab concurrent state update test~~ | High | Added to dashboard-state.service.spec.ts | ✅ Fixed (a58e4372) |
| CR-12 | ~~Redis complete failure test~~ | Medium | Added to dashboard-state.service.spec.ts | ✅ Fixed (a58e4372) |

---

### Code Review Analysis

#### CR-02: Race Condition in Dashboard State Lua Script ✅ RESOLVED

**Issue:** While the Lua script is atomic internally, there's no distributed lock across concurrent save operations. Two clients can both pass the version check and succeed, causing data corruption.

**Fix Applied:** Implemented Redis distributed lock (SETNX) with TTL auto-expiry and Lua script for safe lock release with ownership verification.

#### CR-03: Missing Workspace-Scoped Admin Check ✅ VERIFIED CORRECT

**Issue:** Permission check doesn't verify that `isAdmin` is scoped to the workspace. A user who is admin in Workspace A could cancel approvals in Workspace B.

**Resolution:** Code review found the implementation is already correct. TenantGuard queries WorkspaceMember for the specific user+workspaceId combination, so `isAdmin` is already workspace-scoped. Added clarifying comment to document this security reasoning.

#### CR-04: Pending Changes Lost on Disconnect ✅ INTENTIONAL DESIGN

**Issue:** When socket disconnects, pending changes are dropped.

**Resolution:** Confirmed as intentional design decision:
1. Reconnection triggers full state sync from server
2. Local state is source of truth until sync completes
3. Preserving stale changes could cause conflicts

Future enhancement (optional): Queue changes in localStorage for offline-first experience.

#### CR-05: N+1 Lock Acquisition in Cleanup ✅ FALSE POSITIVE

**Issue:** `cleanup_stale_results()` acquires lock once per stale item inside the loop.

**Resolution:** Code analysis shows lock is acquired ONCE at line 295 and wraps the entire cleanup loop. No N+1 issue exists.

#### CR-06: Missing Backpressure in MCP Connections ✅ RESOLVED

**Issue:** Unlimited concurrent connections via `asyncio.gather()` could exhaust file descriptors on systems with many MCP servers.

**Fix Applied:** Added `asyncio.Semaphore(10)` to limit concurrent MCP connections.

#### CR-07: Memory Leak - No Periodic Cleanup ✅ RESOLVED

**Issue:** `cleanup_stale_results()` exists but is never called periodically. Results accumulate indefinitely if `wait_for_event()` is never called (e.g., agent crash before retrieval).

**Fix Applied:** Added periodic cleanup task (every 10 minutes) in `main.py` startup with proper cancellation on shutdown.
